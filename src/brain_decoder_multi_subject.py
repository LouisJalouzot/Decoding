...
class BrainDecoderMultiSubject(nn.Module):
    def __init__(
        self,
        in_dims: List[int],
        out_dim,
        ...
        # First linear
        self.lin0 = nn.ModuleList(
            [
                nn.Sequential(
                    nn.Linear(dim, hidden_size_backbone),
...
def evaluate(dl, subject, decoder, negatives, top_k_accuracies, temperature):
    decoder.eval()
...
def train_brain_decoder(
    ...
    **decoder_params,
    ...
    # TODO continue
    in_dims = [X.shape[1] for X, _ in data[stories_test[0]]]
    out_dim = data[stories_test[0]][0][1].shape[1]
    train_dl = DataLoader(
    ...
    decoder = BrainDecoderMultiSubject(
        in_dims=in_dims,
        out_dim=out_dim,
        **decoder_params,
    ).to(device)
